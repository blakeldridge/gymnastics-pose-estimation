...
def main(apparatus, folder_path):
    ...
    for frame in frames:
        cam = frame.split("_")[1]
        cam_keypoints = []
        outputs = pose_model(frame, image_loc="device")
    ...
    with open(os.path.join(DIR, f"anim/camera_data/{apparatus}.json")) as f:
        calib = json.load(f)
    ...
    for frame in frames:
        ...
        image_with_joints(frame, keypoints)

if __name__ == "__main__":
    apparatus = input("Apparatus : ")
    folder_path = input("Folder path : ")
    main(apparatus, folder_path)